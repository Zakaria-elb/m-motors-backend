...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, unique + path.extname(file.originalname)); // Nom unique
  },
});
...
// POST /documents (upload d'un fichier pour un dossier)
  ...
  try {
    ...
    const document = await prisma.document.create({
      data: {
        ...
        s3Key: file.filename, // En local, on stocke le nom du fichier
